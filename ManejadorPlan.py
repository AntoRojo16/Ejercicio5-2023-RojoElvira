...
class Planes:
    ...
    def ActualizarValor(self):
        for plan in self.__lista:
            print("Cdigo del plan {}, modelo del vehiculo {}, version del vehiculo {}".format(plan.getCodigo(),plan.getModelo(), plan.getVersion()))
            print("valor del vehiculo {}".format(plan.getValor()))
            valor=input("Ingrese el valor actual del vehiculo")
            plan.ModificarValor(valor)
            print("Se modifico el valor del vehiculo {}".format(plan.getValor()))
